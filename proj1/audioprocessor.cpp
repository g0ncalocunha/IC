...
public:
    ...
    void plotHistogram(){
        if(channelCount >1){
            ...
            int binSize = 256;  // Adjust based on 16-bit sample values
            ...
            sf::RenderWindow window(sf::VideoMode(windowWidth, windowHeight), "Audio Histogram");

            for (auto sample : midChannel) {
                int binIndex = (sample + 32768) / binSize;  // Adjust to handle signed 16-bit samples
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2)
    {
        filename = "audioprocessor_files/yoyok.wav";  
    }
    else
    {
        filename = argv[1];
