...
private:
    ...
    // Modified mapping functions to preserve exact values
...
public:
    ...
    vector<vector<int>> calculateResiduals(const Mat& image, Predictors::Standards standard) {
        ...
        // Store image dimensions in the beginning of first channel
        residuals[0].push_back(rows);
        residuals[0].push_back(cols);

        for (int c = 0; c < image.channels(); ++c) {
            residuals[c].reserve(rows * cols + 2);  // +2 for dimensions
            
            // For channels other than first, also store dimensions
            if (c > 0) {
            ...
            for (int i = 0; i < rows; ++i) {
                for (int j = 0; j < cols; ++j) {
                    ...
                    // Ensure residual is exactly recoverable
    ...
    void encodeWithGolomb(const vector<vector<int>>& residuals, const string& filename) {
        ...
        for (const auto& channelResiduals : residuals) {
            ...
            bs.writeBits(channelResiduals[0], 16);  // rows
            bs.writeBits(channelResiduals[1], 16);  // cols
            
            // Calculate mean excluding the header information
            double mean = 0.0;
            ...
            // Write residuals (skip first two values which are dimensions)
    ...
    vector<vector<int>> decodeWithGolomb(const string& filename) {
        ...
        for (int c = 0; c < numChannels; ++c) {
            ...
            channelResiduals.push_back(rows);  // Store dimensions in residuals
    ...
    Mat reconstructImage(const vector<vector<int>>& residuals, Predictors::Standards standard) {
        // Get dimensions from the residuals
        ...
        for (int c = 0; c < channels; ++c) {
            int idx = 2;  // Start after dimensions
            for (int i = 0; i < rows; ++i) {
                for (int j = 0; j < cols; ++j) {
                    ...
                    // Use modulo arithmetic to handle overflow/underflow
